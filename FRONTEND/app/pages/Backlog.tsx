...
export function Backlog({ tasks, onUpdateTask, onCreateTask, sprints }: BacklogProps) {
  ...
  const getTeamMemberAvatar = (userId: string) => {
    const allMembers = teams.flatMap(t => t.members);
    const member = allMembers.find(m => m.id === userId);
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">Manage and prioritize your product backlog items</p>
          </div>
            ...
            >
              <div className="flex items-start gap-4">
                  ...
                  {task.assigneeId && (
                    <div className="flex items-center gap-2 mt-2">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#30c2b7] to-[#70e1bf] flex items-center justify-center text-white text-xs">
                        {getTeamMemberAvatar(task.assigneeId)}
                      </div>
                      <span className="text-sm text-gray-600 dark:text-gray-400">{getTeamMemberName(task.assigneeId)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="w-48" onClick={(e) => e.stopPropagation()}>
                  <Select
                    value={task.sprintId || ''}
                    onValueChange={(value) => handleSprintAssignment(task.id, value)}
                  >
                    <SelectTrigger>
